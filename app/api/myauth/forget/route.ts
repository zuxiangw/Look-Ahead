...
import { insertToken, searchUserByNameNEmail } from "@/app/utils/database";
...
async function performForget(email: string) {
  const ret_query = (await searchUserByNameNEmail(
    undefined,
    email
  )) as RowDataPacket[];
  ...
  if (ret_query.length !== 0) {
    ...
    try {
      ...
      const body = `You have requested to reset your password for Look Ahead. Please head to the following link to reset your password: http://localhost:3000/auth/forget/${reset_token}\n`;
